$(function() {
    ...
    /**
     * Prepare history-enabled inputfields by injecting required GUI elements etc.
     */
    window.versionControlPrepareInputfields = function() {
        ...
        $('#version-control-data > div:not(.version-control--prepared)').each(function() {
        ...
        $('.version-control--with-history:not(.version-control--prepared)').each(function() {
        ...
        $('.field-revisions')
            .off('click.revisions-restore')
            .on('click.revisions-restore', '.field-revision__button--restore, .field-revision__current', function() {
        ...
        $('.field-revisions-toggle')
            .off('click.toggleRevisions')
            .on('click.toggleRevisions', function(event) {
        ...
        $(window)
            .off('keyup.field-revisions')
            .on('keyup.field-revisions', function(event) {
        ...
        $('.field-revisions > div')
            .off('scroll.revisions-table')
            .on('scroll.revisions-table', function() {
        ...
        $('.field-revisions')
            .off('click.revisions-diff')
            .on('click.revisions-diff', '.field-revision__button--diff', function() {
                $('.compare-revisions').remove();
    ...
    var enableDiffMatchPatch = function(r1, r2) {
        ...
        if (r1 == r2) {
            ...
            if (typeof diff_match_patch != 'function') {
                $.getScript(moduleConfig.moduleDir + "res/js/diff_match_patch/diff_match_patch.js", function() {
    ...
    $.get(moduleConfig.processPage + 'page', { pages_id: moduleConfig.pageID, settings: settings }, function(data) {
        ...
        // Prepare inputfields.
        versionControlPrepareInputfields();

        // AJAX loaded inputfields.
